...
export async function GET(req: NextRequest) {
  try {
    ...
    // State is an opaque server-generated CSRF nonce. Never include caller-controlled
    // identity data: the callback uses this value only to bind the browser flow.
    ...
    return NextResponse.json(
      { error: "Failed to start Shopify OAuth flow." },
      { status: 500 }
    );
  }
}
