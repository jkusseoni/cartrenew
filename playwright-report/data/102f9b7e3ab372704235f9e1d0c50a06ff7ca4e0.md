...
- Name: auth.spec.ts >> auth routes >> login submits credentials and redirects or shows auth feedback
...
Call log:
  - navigating to "http://localhost:3000/sign-in", waiting until "domcontentloaded"
...
> 46  |     await page.goto(loginPath, { waitUntil: "domcontentloaded" });
  ...
  139 |           rect.width > 0 &&
  140 |           rect.height > 0 &&
  141 |           style.visibility !== "hidden" &&
  142 |           style.display !== "none"
  143 |         );
  144 |       }
  145 |     },
  146 |     { email: data.email, password: data.password }
```